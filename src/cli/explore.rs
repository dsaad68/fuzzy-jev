...
//! Only a request whose `Host` is this address, and which carries the `X-Jev` header a form on
//! another site can't send without asking first, is answered, so no other page open in the same
//! browser can spend the key.
...
/// Opens `address` in the default browser, or says to.
fn open(address: &str) {
    ...
    if !command.is_ok_and(|status| status.success()) {
...
async fn connection(mut stream: TcpStream, server: &Server) -> std::io::Result<()> {
    let (status, kind, body) = match request(&mut stream).await? {
...
fn head(bytes: &[u8]) -> Result<(Request, usize), &'static str> {
    ...
    let mut request = Request { method: method.to_owned(), path: path.to_owned(), host: None, origin: None, jev: false, body: Vec::new() };
    let mut length = 0;
    for line in lines {
...
async fn respond(request: &Request, server: &Server) -> (u16, &'static str, Vec<u8>) {
    ...
    match (request.method.as_str(), path) {
        ...
        ("GET", "/start") => (200, "application/json", server.start.to_string().into_bytes()),
        ("POST", "/run") if request.jev => {
        ...
        ("POST", "/run") => (403, "text/plain; charset=utf-8", b"not this server's page".to_vec()),
...
mod tests {
    ...
    fn post(host: &str, origin: Option<&str>, jev: bool, body: Value) -> Request {
        Request {
    ...
    async fn answers_only_its_own_page() {
        ...
        assert_eq!(status(post("127.0.0.1:7000", Some("http://127.0.0.1:7000"), true, draw.clone())).await, 200);
        assert_eq!(status(post("localhost:7000", None, true, draw.clone())).await, 200);
        // Another site's form can't add the header; a rebound name or another origin is refused.
        assert_eq!(status(post("127.0.0.1:7000", None, false, draw.clone())).await, 403);
        assert_eq!(status(post("evil.example:7000", None, true, draw.clone())).await, 403);
        assert_eq!(status(post("127.0.0.1:7000", Some("http://evil.example"), true, draw)).await, 403);
    }
